...
const resolveVersionMetadata = (props) =>
  props.route?.customProps ?? props.customProps ?? props;

export default function ApiReferencePage(props) {
  const versionMetadata = resolveVersionMetadata(props);
  const openApiUrl = useBaseUrl(`/openapi/${versionMetadata.versionSlug}/`);
  ...
  return (
    <Layout
      title={`API Reference ${versionMetadata.versionSlug}`}
      description={`API Reference Documentation for ${versionMetadata.versionSlug}`}
    >
      <iframe
        ref={iframeRef}
        src={initialSrc}
        ...
        title={`OpenAPI Documentation ${versionMetadata.versionSlug}`}
      />
    </Layout>
  );
}
